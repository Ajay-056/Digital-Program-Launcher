document.addEventListener("DOMContentLoaded", () => {
  ...
  const launched_by = document.querySelector("#launched_by");
  ...
  const updateParticipantList = (participants) => {
    ...
    if (participants.length > 0)
      statusMessage.textContent = `Launch in progress... ${participants.length} Leader(s) Launched`;
  };
  ...
  const startCountdown = (duration, imageUrl) => {
    ...
    statusMessage.textContent = "Going to launch in...";
    ...
    const updateTimer = () => {
      ...
      if (timer-- <= 0) {
        ...
        heading.style.display = "none"; // Hide the main heading
        countdownTimer.style.display = "none";
        statusMessage.textContent = "";
        showLaunchImage(imageUrl);
        startConfetti();
        launched_by.classList.remove("hidden");
        launched_by.textContent = "1. Hari 2. Leo 3. Uma";
      }
    };
  ...
  const fetchInitialState = async () => {
    try {
      ...
      if (data.participants.length < data.target) {
        ...
        startCountdown(
          data.countdown,
          "https://raw.githubusercontent.com/Ajay-056/Digital-Program-Launcher/main/Screenshot%202025-12-18%20193648.png"
        );
      }
    } catch (error) {
